...
export async function buildPlatformStatus(env){
  ...
  return {
    ok,ready:ok&&schedulerHealthy,service:'ronda-one',version:'0.9.7.5.10',database,schedulerHealthy,lastSuccessAt,generatedAt,
    queues,
  ...
  async fetch(request,env,ctx){
    ...
    if(url.pathname==='/api/platform/status'){ const operational=await buildPlatformStatus(env); return json({
      ...
      uiRecovery:{
        ...
        assetCacheBust:'2.9.7.5.10-quality-first-cost-crawl'
      },
      navigation:{
        ronda:'/ronda',
        crawl:'/ronda#crawl',
