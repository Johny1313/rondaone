...
export async function buildPlatformStatus(env){
  ...
  return {
    ok,ready:ok&&schedulerHealthy,service:'ronda-one',version:'0.9.7.6.1',database,schedulerHealthy,lastSuccessAt,generatedAt,
    queues,
...
export default {
  async fetch(request,env,ctx){
    ...
    if(url.pathname==='/api/platform/status'){ const operational=await buildPlatformStatus(env); return json({
      ...
      carouselStabilityBaselineDefinitive:{enabled:true,engineBaseline:'0.9.7.5.6',reader:'hybrid-reader-v1.1-band-v2',evidenceVersion:'ronda-evidence-pack-v1-reader-v1.1',pipelineVersion:'carousel-stability-baseline-v1.1',productionRecoveryCadenceMinutes:1,roundCadenceMinutes:5,browserPolicy:'fallback-only-normal-path',singleBrowserNavigation:true,legacyCacheReuse:false,legacyActiveJobReuse:false,d1Delete:false,qualityPipelinePreserved:true,costGovernorPreserved:true},
      ...
      uiRecovery:{
        ...
        assetCacheBust:'2.9.7.6.1-band-reader-v2-retry-order'
      },
      navigation:{
        ronda:'/ronda',
        crawl:'/ronda#crawl',
