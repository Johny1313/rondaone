...
export async function processProductionRead(env,jobId,{force=false,retryMode=null}={}){
  ...
  try{
    ...
    if(job.sourceType==="url"){
      ...
      const transportPreference=retryMode==="deep"&&browserFetcher?"browser-first":"direct-first";
      ...
      evidenceResult=await scrapeTopicToEvidence(topic,{
        ...
        transportPreferenceFor:async()=>(retryMode==="deep"&&browserFetcher?"browser-first":"direct-first"),
...
export async function retryProductionJob(env,id,{ctx=null,stage=null}={}){
  ...
  const labels={alternate:'leitura direta renovada + adapter/AMP antes do Browser',deep:'leitura profunda com Browser prioritário',snapshot:'snapshot/cache + rotas disponíveis'};
