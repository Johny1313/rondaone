...
function structuredBodyText(value, depth=0) {
  if(depth>5||value==null)return "";
  if(typeof value==="string")return plainText(value);
  if(Array.isArray(value))return plainText(value.map((entry)=>structuredBodyText(entry,depth+1)).filter(Boolean).join("\n\n"));
  if(typeof value!=="object")return "";
  for(const key of ["articleBody","transcript","text","articleContent","content","description"]){
    const text=structuredBodyText(value?.[key],depth+1);
    if(wordCount(text)>=MIN_USEFUL_WORDS)return text;
  }
  return "";
}
...
function schemaTypes(node) {
  ...
  return raw.map((type)=>String(type||"").split(/[\/#]/).pop()).filter(Boolean);
}
...
function extractJsonLdFastArticle(html) {
  ...
  for (const script of scripts.slice(0, 20)) {
    let raw = script.replace(/^<script\b[^>]*>/i, "").replace(/<\/script>$/i, "").trim().replace(/^<!--|-->$/g,"").trim().replace(/;\s*$/,"");
    ...
    for (const node of jsonLdNodes(parsed)) {
      ...
      const isArticle=types.some((type)=>/^(NewsArticle|Article|Reportage|AnalysisNewsArticle|BlogPosting)$/i.test(type));
      const isVideo=types.some((type)=>/^VideoObject$/i.test(type));
      if(!isArticle&&!isVideo)continue;
      const primary=isVideo?(node?.transcript||node?.articleBody||node?.text||node?.description):(node?.articleBody||node?.text||node?.articleContent||node?.mainEntity);
      const content=structuredBodyText(primary);
      ...
      const candidate={
        ...
        publishedAt:plainText(node.datePublished||node.dateModified||node.uploadDate)||null,
        imageUrl:jsonLdImage(node.image||node.thumbnailUrl),
        method:isVideo?(node?.transcript?"json-ld-video-transcript":"json-ld-video-description"):"json-ld-fast",
        pageKind:isVideo?"video":"article",
      };
      if(!best||candidate.wordCount>best.wordCount)best=candidate;
    }
  }
  return best;
}
...
export const PORTAL_ADAPTERS = Object.freeze([
  ...
  { id:"band", label:"Band", version:"v2", hosts:["band.com.br"], signals:[/article-content/i,/article__content/i,/article-text/i,/content-article/i,/post-content/i,/post-body/i,/entry-content/i,/news-content/i,/news-body/i,/story-body/i,/story-content/i,/article-body/i,/materia/i,/rich-text/i,/content-body/i] },
]);
...
function decodeJsonStringLiteral(value) {
  ...
  catch{return raw.slice(1,-1).replace(/\\n/g,"\n").replace(/\\r/g,"\n").replace(/\\t/g," ").replace(/\\"/g,'"').replace(/\\\\/g,"\\");}
}

function bandEmbeddedContent(html) {
  const source=String(html||"");
  let best="";
  const patterns=[
    /["'](?:articleBody|articleContent|transcript)["']\s*:\s*("(?:\\.|[^"\\]){80,}")/gi,
    /["'](?:content|body|text)["']\s*:\s*("(?:\\.|[^"\\]){120,}")/gi,
  ];
  for(const pattern of patterns){
    let match;let count=0;
    while((match=pattern.exec(source))&&count<24){
      count+=1;
      const decoded=decodeJsonStringLiteral(match[1]);
      const text=/<p\b/i.test(decoded)?cleanParagraphs(decoded):plainText(decodeEntities(decoded));
      if(wordCount(text)>wordCount(best))best=text;
      if(wordCount(best)>=900)break;
    }
    if(wordCount(best)>=900)break;
  }
  return best.slice(0,MAX_ARTICLE_CHARS);
}

function adapterExtract(html, url) {
  const adapter = portalAdapterForUrl(url);
  if (!adapter) return null;
  let best = adapter.id==="band"?bandEmbeddedContent(html):"";
  ...
  return {
    ...
    method: `adapter:${adapter.id}${adapter.version?`-${adapter.version}`:""}`,
    adapter: adapter.id,
  ...
  try {
    const response = await fetcher(url, {
      ...
      headers: {
        ...
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/151 Safari/537.36 RondaOne/0.9.7.4.8",
      },
    });
...
function readingQualityScore(record) {
  ...
  const structuredRoute=/^(?:adapter:|json-ld)/.test(record.extractionMethod || "");
  if (structuredRoute) score += 5;
  if (record.evidenceSufficiency?.ready) score += 12;
  if (record.evidenceSufficiency?.ready && structuredRoute) score=Math.max(score,58);
