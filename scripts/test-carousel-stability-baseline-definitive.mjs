...
assert.equal(pkg.version,"0.9.7.6.1");
...
// O article-reader consolidado permanece congelado; o scraping-engine recebeu somente Band v2/JSON-LD estruturado.
...
assert.match(scraping,/id:"band"[\s\S]*version:"v2"[\s\S]*hosts:\["band\.com\.br"\]/);
assert.match(scraping,/bandEmbeddedContent/);
assert.match(scraping,/VideoObject/);
...
// Normal path e retry 1 continuam direct-first; Browser só é priorizado no retry 2 deep.
