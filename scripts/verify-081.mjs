...
check('version 0.8.1',pkg.version==='0.8.1');
...
check('shell 0.8.1',shell.includes("PLATFORM_VERSION='0.8.1'"));
check('asset cache 081',index.includes("assetCacheBust:'2.8.5-081-source-recovery'"));
...
check('cron every 3 minutes',wrangler.includes('"*/3 * * * *"'));
check('high frequency source 3 minutes',collector.includes('if (HIGH_FREQUENCY_SOURCES.has(id)) return 3;'));
check('healthy source max 5 minutes',collector.includes('if (MEDIUM_FREQUENCY_SOURCES.has(id)) return 5;'));
check('last good route first',collector.includes('function orderedFeedUrls')&&collector.includes('orderedFeedUrls(feed, sourceState)'));
check('304 is healthy',collector.includes('successfulResponses += 1;')&&collector.includes('if (response.status === 304)'));
check('no-new is healthy',collector.includes('if (successfulResponses > 0)'));
check('old 1h and 6h freezes removed',!collector.includes('return 360;')&&!collector.includes('return 60;'));
check('failed source max silence 10 minutes',collector.includes('failedMaxSilenceMinutes = 10'));
check('platform source recovery telemetry',index.includes('sourceRecovery:{')&&index.includes('longSourceBackoffRemoved:true'));

let failed=0;
for(const [name,ok] of checks){console.log(`${ok?'OK':'FAIL'} - ${name}`);if(!ok)failed++;}
if(failed){console.error(`\n${failed} verificação(ões) falharam.`);process.exit(1);}

for(const script of ['test-080-functional.mjs','test-080-editorial.mjs','test-080-stability.mjs','test-081-source-recovery.mjs']){
  const run=spawnSync(process.execPath,[path.join(root,'scripts',script)],{encoding:'utf8'});
  process.stdout.write(run.stdout||''); process.stderr.write(run.stderr||'');
  if(run.status!==0) process.exit(run.status||1);
}
console.log(`\nRONDA ONE Cloud v0.8.1: ${checks.length} verificações + 4 rodadas de teste OK.`);
